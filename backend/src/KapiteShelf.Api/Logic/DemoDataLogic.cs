...
public class DemoDataLogic
{
    ...
    public DemoDataLogic(IDbContextFactory<KapitelShelfDBContext> dbContextFactory)
    {
    ...
    public async Task GenerateAsync()
    {
        ...
        {
            var bookFaker = this.CreateBookFaker();
    ...
    private Faker<BookModel> CreateBookFaker()
    {
        var seriesFaker = this.CreateSeriesFaker();
        ...
        return new Faker<BookModel>()
            ...
            .RuleFor(x => x.Series, x => this.CreateSeriesFaker().Generate())
            .RuleFor(x => x.SeriesId, (f, u) => u.Series!.Id)
            .RuleFor(x => x.SeriesNumber, x => x.Random.Number(1, 10))

            .RuleFor(x => x.Author, x => this.CreateAuthorFaker().Generate())
            .RuleFor(x => x.AuthorId, (f, u) => u.Author!.Id)
            
            .RuleFor(x => x.Categories, (f, u) => this.CreateBookCategoryFaker(u.Id).GenerateBetween(1, 2).ToList())

            .RuleFor(x => x.Tags, (f, u) => this.CreateBookTagFaker(u.Id).GenerateBetween(1, 5).ToList())

            .RuleFor(x => x.Cover, x => this.CreateFileInfoFaker().Generate())
            .RuleFor(x => x.Location, x => this.CreateLocationFaker().Generate());
    }

    private Faker<SeriesModel> CreateSeriesFaker()
    {
    ...
    private Faker<AuthorModel> CreateAuthorFaker()
    {
    ...
    private Faker<BookCategoryModel> CreateBookCategoryFaker(Guid bookId)
    {
        return new Faker<BookCategoryModel>()
            .RuleFor(x => x.BookId, x => bookId)

            .RuleFor(x => x.Category, x => this.CreateCategoryFaker().Generate())
            .RuleFor(x => x.CategoryId, (f, u) => u.Category.Id);
    }

    private Faker<CategoryModel> CreateCategoryFaker()
    {
    ...
    private Faker<BookTagModel> CreateBookTagFaker(Guid bookId)
    {
        return new Faker<BookTagModel>()
            .RuleFor(x => x.BookId, x => bookId)

            .RuleFor(x => x.Tag, x => this.CreateTagFaker().Generate())
            .RuleFor(x => x.TagId, (f, u) => u.Tag.Id);
    }

    private Faker<TagModel> CreateTagFaker()
    {
    ...
    private Faker<FileInfoModel> CreateFileInfoFaker()
    {
    ...
    private Faker<LocationModel> CreateLocationFaker()
    {
        return new Faker<LocationModel>()
            .RuleFor(x => x.Id, x => x.Random.Guid())

            .RuleFor(x => x.Type, x => x.Random.Enum<LocationTypeEnum>())

            .RuleFor(x => x.Url, x => x.Internet.Url())
            .RuleFor(x => x.FileInfo, x => this.CreateFileInfoFaker().Generate());
    }
}
